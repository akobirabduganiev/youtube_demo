...
public class LikeController {
   ...
   /* @PutMapping("/{id}")
    ...
    public ResponseEntity<?> findAllByVideoId(@PathVariable("id") Integer articleId,
                                              @RequestParam(value = "page", defaultValue = "0") int page,
                                              @RequestParam(value = "size", defaultValue = "3") int size) {
        return ResponseEntity.ok(likeService.listByVideoId(articleId, page, size));
    }
    ...
    public ResponseEntity<?> findByProfile(@PathVariable("id") Integer articleId,
                                           HttpServletRequest request) {
        Integer pId = JwtUtil.getIdFromHeader(request);
        return ResponseEntity.ok(likeService.getVideoById(articleId, pId));
    }
